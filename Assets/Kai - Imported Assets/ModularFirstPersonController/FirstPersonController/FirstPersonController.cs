...
public class FirstPersonController : MonoBehaviour
{
    ...
    public bool doesPlayerWantHeadBob = false;
    public Transform joint;
    public float bobSpeed = 10f;
    public Vector3 bobAmount = new Vector3(.15f, .05f, 0f);

    // Internal Variables
    private bool canHeadBob = false;
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            if (enableSprint && sprintPressed && sprintRemaining > 0f && !isSprintCooldown)
            {
                ...
                // If the player is actually moving enable headbob, if not disable headbob to prevent unwanted movement while idle
    ...
    {
        if(isWalking && doesPlayerWantHeadBob)
        {
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        fpc.doesPlayerWantHeadBob = EditorGUILayout.ToggleLeft(new GUIContent("Enable Head Bob", "Determines if the camera will bob while the player is walking."), fpc.doesPlayerWantHeadBob);
        //fpc.canHeadBob = EditorGUILayout.ToggleLeft(new GUIContent("Enable Head Bob", "Determines if the camera will bob while the player is walking."), fpc.canHeadBob);
        
        GUI.enabled = fpc.doesPlayerWantHeadBob;
